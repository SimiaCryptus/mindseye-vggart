...
class OptimizerDemo extends WordSpec with MustMatchers with ReportNotebook {
    ...
    "OWL-QN" in {
      report("owlqn", log ⇒ {
        test(log, trainable ⇒ log.eval {
          ...
          trainer.setLineSearchFactory((s:String)⇒new ArmijoWolfeSearch().setC2(0.8).setAlpha(1e-6))
          trainer
        })
      })
    }
  ...
  private def getBlankDeltaSet(model: PipelineNetwork) = {
    ...
    model.eval(new NNExecutionContext {}, new Tensor(inputSize: _*))
      .accumulate(set, new TensorArray(new Tensor(outputSize: _*)))
    set
  }
