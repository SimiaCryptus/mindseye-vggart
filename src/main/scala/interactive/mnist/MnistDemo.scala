...
class MnistDemo(server: StreamNanoHTTPD, log: HtmlNotebookOutput with ScalaNotebookOutput) extends MindsEyeNotebook(server, log) {
  ...
  def phase1() = phase({
    ...
    log.eval {
      def wrap(n: Layer) = new MonitoringWrapperLayer(n).addTo(monitoringRoot)
  ...
  def phase2() = phase("mnist_initialized", (model: Layer) ⇒ {
  ...
  def validateModel(log: HtmlNotebookOutput with ScalaNotebookOutput, model: Layer) = {
    log.h2("Validation")
